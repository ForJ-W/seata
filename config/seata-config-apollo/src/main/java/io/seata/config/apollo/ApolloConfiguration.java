...
public class ApolloConfiguration extends AbstractConfiguration {
    ...
    // 修改系统prop apolloMeta为apollo.meta appId为app.id 添加环境配置
    private static final String APP_POINT_ID = "app.id";
    private static final String APOLLO_POINT_META = "apollo.meta";
    ...
    public String getConfig(String dataId, String defaultValue, long timeoutMills) {
        ...
        configOperateExecutor.submit(() -> {
            ...
            //在seata client使用apollo时 若namespace与client项目中的namespace一致,会去读取项目apollo缓存配置
    ...
    private void readyApolloConfig() {
        ...
        if (!properties.containsKey(APP_ID)) {
            ...
            System.setProperty(APP_POINT_ID, FILE_CONFIG.getConfig(getApolloAppIdFileKey()));
            LOGGER.info("System.setProperty(APP_ID): {}",FILE_CONFIG.getConfig(getApolloAppIdFileKey()));
        }
        if (!properties.containsKey(APOLLO_META)) {

            System.setProperty(APOLLO_POINT_META, FILE_CONFIG.getConfig(getApolloMetaFileKey()));
            LOGGER.info("System.setProperty(APOLLO_META): {}",FILE_CONFIG.getConfig(getApolloMetaFileKey()));
        }
        if (!properties.containsKey(APOLLO_ENV)) {

            System.setProperty(APOLLO_ENV, FILE_CONFIG.getConfig(getApolloENV()));
            LOGGER.info("System.setProperty(APOLLO_ENV): {}",FILE_CONFIG.getConfig(getApolloENV()));
        }
    }
    ...
    private static String getApolloENV() {
        return String.join(FILE_CONFIG_SPLIT_CHAR, FILE_ROOT_CONFIG, REGISTRY_TYPE, APOLLO_ENV);
    }
    ...
    private static String getApolloNamespaceKey() {
        String apolloNamespaceKey = String.join(FILE_CONFIG_SPLIT_CHAR, FILE_ROOT_CONFIG, REGISTRY_TYPE, NAMESPACE);
        return apolloNamespaceKey;
    }
